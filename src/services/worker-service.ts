...
import { getWorkerPort, getConfiguredWorkerPort, getWorkerHost, fetchWithTimeout, resolveWorkerScriptPath, clearPortCache, WORKER_PORT_WALK_RANGE } from '../shared/worker-utils.js';
...
export class WorkerService implements WorkerRef {
  ...
  async start(): Promise<void> {
    // WINDOWS-FORK: the daemon's walk base is the settings-resolved port, NOT
    // the pidfile-preferring getWorkerPort() — a stale-but-alive pidfile must
    // not seed the daemon's own bind attempt.
    ...
    // WINDOWS-FORK: port-walk on EADDRINUSE. The configured port can be held
    // by a Windows kernel TCP zombie (dead-PID LISTEN socket that nothing
    // short of a reboot clears) or by another worker. Try [configured ..
    // configured+WORKER_PORT_WALK_RANGE-1], bind the first free candidate,
    // and write the ACTUAL bound port to the pidfile — clients resolve the
    // port pidfile-first (getWorkerPort), so a walked port is discovered
    // automatically. Server.listen constructs a fresh http.Server per call,
    // so retrying after a rejected bind is safe.
    ...
    for (let attempt = 0; ; attempt++) {
      try {
        ...
        if (code !== 'EADDRINUSE' || attempt >= WORKER_PORT_WALK_RANGE - 1) {
          throw error;
        }
    ...
    // In-process port resolution must now see the pidfile's bound port, not a
    // value cached before the walk.
    clearPortCache();
...
async function main() {
  ...
  switch (command) {
    ...
    default: {
      ...
      if (await isPortInUse(port)) {
        // WINDOWS-FORK (ledger B1): bound does not mean duplicate. A dead-PID
        // ghost LISTEN socket binds the port but answers nothing; refusing
        // here would leave NO worker at all. Only a health answer proves a
        // live duplicate — anything else falls through, and the start()
        // port-walk binds the next free candidate.
        if (await waitForHealth(port, 1500)) {
          logger.info('SYSTEM', 'Port already in use, refusing to start duplicate', { port });
          process.exit(0);
        }
        logger.warn('SYSTEM', 'Port bound but health unreachable (phantom/ghost or foreign owner) — starting anyway; daemon will walk past it', { port });
      }
